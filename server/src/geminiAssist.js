...
每条约 18～52 字为宜，五句之间句式要有变化。`;
    const user = `${ctxBlock}\n\n请写 5 条延伸线索（中文），彼此角度不同，读起来要像参考工具里「I wonder…」那种——顺着当前笔记，自然往外多开几扇窗（人物、台词、桥段、演员、同类对比等均可），但**落点要具体**。

反例：五条都只谈「偶像剧叙事/服化道」却不提笔记里写的那部作品。
正例（当笔记在聊某部剧）：「夏之星和仲天骐这条线，几次大的误会分别是怎么收束的？」「剧里那句特别出圈的台词，当时是在什么情境下说的？」「和同时期几部台偶比，这部在节奏上最明显的特点是什么？」

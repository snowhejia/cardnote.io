...
export async function runNoteAssist(payload) {
  ...
  if (task === "suggest_questions") {
    const sys =
      `${weightHint}${visionHint} 你是学习助手。根据笔记生成 5 条「延伸探索」：每一条都是用户即将发给下游 AI 的短主题，用来多查信息（演员、幕后、同类推荐、制作背景等），不是布置长文写作任务，也不是在问用户本人。回复必须且仅为一个 JSON 对象，不要 Markdown 代码围栏，不要其它说明文字。键 questions 为长度恰好 5 的字符串数组（字段名仍为 questions 以兼容客户端）。

人称与语气：用无主句或「某某 + 信息角度」的短标题式句子，像资讯栏目或搜索关键词；严禁出现：你觉得、你认为、你怎么看、你是否、对你来说、对你而言、请问你、你最喜欢 等面向读者的第二人称追问。

具体性（极重要）：若笔记出现具体作品、人物、产品、软件名、事件，优先直呼其名；至少 3 条明显扣住笔记里的具体对象。少写空泛大类。

延伸形态（极重要）：优先像「作品名 + 信息栏目」这类可检索方向，例如演员/主创介绍、幕后花絮与制作、同类型作品或竞品推荐、原声带与名场面、拍摄地与改编来源；工具/产品类则类似：功能要点、定价与竞品、同类软件对比、更新与口碑信息。句子要短，像用户举例：「放羊的星星演员介绍」「放羊的星星幕后花絮」「放羊的星星同类型剧推荐」——可略加一字两字衔接，但不要写成一整段论述句。

主题优先，不写「作者心理」：延伸须落在笔记里提到的客观主题（某剧、某书、某工具），禁止把整条写成对写笔记的人的心理分析。

开放 vs 考据：不要封闭式 trivia（第几句台词、谁第几集说的）；工具类也不要追问个人使用感受。

禁止：把笔记作者称作「用户」「笔者」；禁止问卷式排比。

每条约 8～36 字为宜（偏短、好点的主题），五条之间角度要岔开（演员 / 幕后 / 同类 / 配乐或制作 / 其它信息增量）。`;
    const user = `${ctxBlock}\n\n请写 5 条延伸线索（中文），彼此角度不同。每条都是「多查这类信息」的短主题，不要写成分析读者情绪或付费心理的长 brief。

反例 A：通篇只写「偶像剧」却不带笔记里的具体剧名或片名。
反例 B：封闭式考据 trivia。
反例 C：「你觉得…？」类。
反例 D：大段「从叙事结构论…」式论文题目——改短，改成上面「作品名 + 演员介绍 / 幕后花絮 / 同类型推荐」这种信息向栏目。

正例·影视（风格参考，请按笔记实际作品名替换）：放羊的星星演员介绍；放羊的星星幕后花絮；放羊的星星同类型剧推荐。

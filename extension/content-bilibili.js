...
async function scrapeBilibiliVideoPageAsync(mainWorldPlay) {
  ...
  /** 分P/选集：pages 数组，每一项含 cid / part（选集标题）/ duration（选集时长） */
  let viewApiPagesRaw = null;
  if (bvid) {
    try {
      const r = await fetch(
      ...
      if (r.ok) {
        ...
        if (j && j.code === 0 && j.data) {
          ...
          if (Array.isArray(j.data.pages) && j.data.pages.length > 0) {
            viewApiPagesRaw = j.data.pages;
          }
        }
      }
    } catch {
  ...
  /** 分P/选集：优先用 URL ?p= 参数（用户当前选集的权威来源），
   *  然后才是 __playinfo__（播放器实时态）；__INITIAL_STATE__ 内联 JSON 的 cid
   *  可能是 pages[0].cid（第 1 集），不可靠，最后兜底。 */
  let partTitle = "";
  let partDurationSec = 0;
  if (viewApiPagesRaw && viewApiPagesRaw.length > 1) {
    let partIdx = -1;
    const pMatch = location.search.match(/[?&]p=(\d+)/);
    if (pMatch) {
      const p = Number(pMatch[1]);
      if (Number.isFinite(p) && p > 0 && p <= viewApiPagesRaw.length) {
        partIdx = p - 1;
      }
    }
    if (partIdx < 0 && mainWorldPlay && mainWorldPlay.cid != null) {
      const mwCid = Number(mainWorldPlay.cid);
      if (Number.isFinite(mwCid) && mwCid > 0) {
        const i = viewApiPagesRaw.findIndex(
          (pg) => Number(pg && pg.cid) === mwCid
        );
        if (i >= 0) partIdx = i;
      }
    }
    if (partIdx < 0) partIdx = 0;
    const page = viewApiPagesRaw[partIdx];
    const pt = sanitizeBilibiliTitle(
      typeof page?.part === "string" ? page.part : ""
    );
    /** 选集名单独返回，用于视频附件文件名；卡片标题仍用稿件主标题，
     *  便于同一稿件不同选集合并到同一张卡片。 */
    if (pt && pt !== title) partTitle = pt;
    const pd = Number(page?.duration);
    if (Number.isFinite(pd) && pd > 0) partDurationSec = Math.round(pd);
    /** 用 pages 里的权威 cid，避免 __INITIAL_STATE__ 内联 JSON 抓到的是 pages[0]
     *  导致 playurl 回退拉到第 1 集而不是当前选集 */
    const pc = Number(page?.cid);
    if (Number.isFinite(pc) && pc > 0) clipCid = pc;
  }
  ...
  let dashUrls = null;
  try {
    /** 选集视频用 clipCid（已按 pages 匹配覆盖），避免 playurl 回退拉到错误分 P */
    dashUrls = await resolveBiliDashUrls(
      fromJson.bvid || bvid,
      clipCid || fromJson.cid,
      mainWorldPlay
    );
  } catch {
    dashUrls = null;
  ...
  /** 卡片时长仍用稿件主时长；选集时长通过 partDurationSec 单独返回，
   *  后续只挂在附件文件上，避免多选集合并到同一卡片时覆盖主时长。 */
  const durationSec =
  ...
  return {
    ...
    publishDateYmd,
    durationSec,
    /** 选集标题与时长：用于同稿件多选集合并到同一卡片时命名附件文件、挂 file 元数据 */
    partTitle: partTitle || "",
    partDurationSec: partDurationSec > 0 ? partDurationSec : null,
  };
}

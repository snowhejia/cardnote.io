...
2. 「加载已解压的扩展程序」→ 选择本目录 `extensions/xhs-to-cardnote`。
...
| Bearer Token | 一般为 Local Storage 的 `cardnote_admin_jwt`；或服务端 `API_TOKEN` |
...
3. 视频若超过扩展上传上限（约 1024MB），会跳过上传并提示你自行下载后从网页版添加附件。

...
/**
 * MV3 service worker 可用 createImageBitmap 解码图片拿到像素尺寸；
 * 解码失败/格式不支持时返回 null，调用方据此选择不带 widthPx/heightPx。
 */
async function probeImageDimensions(blob) {
  try {
 ...
 */
async function fetchAsBlobViaBiliVideoTab(tabId, mediaUrl, onProgress) {
  const u = String(mediaUrl || "").trim();
  if (!u || tabId == null) throw new Error("缺少地址或标签页");
  const dlId = `cn_${Date.now().toString(36)}_${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  ...
  try {
    await chrome.scripting.executeScript({
      ...
      args: [u, dlId],
      func: (mediaU, id) => {
        const bag = (window.__cnDl = window.__cnDl || {});
        const slot = (bag[id] = {
          done: false,
          loaded: 0,
          total: 0,
        ...
        (async () => {
          try {
            ...
            slot.contentLength = cl;
            slot.total = cl ? Number(cl) || 0 : 0;
            slot.contentType = res.headers.get("content-type") || "";
            if (!res.ok) {
              slot.done = true;
              return;
            }
            ...
            for (;;) {
              ...
              if (value) {
                chunks.push(value);
                slot.loaded += value.byteLength;
              }
            }
            const out = new Uint8Array(slot.loaded);
            let off = 0;
            ...
            slot.err = String(err?.message || err || "fetch failed");
            slot.done = true;
          }
        })();
      },
  ...
  for (;;) {
    ...
    try {
      pollRes = await chrome.scripting.executeScript({
        ...
        args: [dlId],
        func: (id) => {
          const s = (window.__cnDl || {})[id];
          if (!s) return { missing: true };
          if (!s.done) {
            return { done: false, loaded: s.loaded, total: s.total };
          }
          const out = {
            done: true,
            loaded: s.loaded,
    ...
    const raw = pollRes?.[0]?.result;
    if (!raw || typeof raw !== "object") {
      throw new Error("页面内下载无返回");
    }
    if (raw.missing) throw new Error("页面内下载状态丢失（页面可能已刷新）");
    if (typeof onProgress === "function") {
      try {
        onProgress(raw.loaded || 0, raw.total || 0);
      } catch {}
    }
    if (raw.done) {
      final = raw;
  ...
  const status = Number(final.status) || 0;
  if (status > 0 && !(status >= 200 && status < 300)) {
    throw new Error(explainHttpStatus(status));
  }
  if (final.err) throw new Error(final.err);
  const buf = final.buf;
  if (!(buf instanceof ArrayBuffer) || buf.byteLength < 1) {
    throw new Error("页面内下载结果为空");
  }
  assertBufferMatchesContentLength(buf.byteLength, final.contentLength);
  const ct =
    typeof final.contentType === "string" && final.contentType.trim()
      ? final.contentType.trim()
      : "application/octet-stream";
  return new Blob([buf], { type: ct });
}

/** 音画 CDN：先页内 MAIN，失败再扩展 SW（双保险） */
async function fetchBiliCdnBlobDualContext(tabId, url, referer, onProgress) {
  const media = String(url || "").trim();
  if (!media) throw new Error("缺少下载地址");
  if (tabId != null && shouldSendBilibiliStreamCookies(media)) {
    try {
      return await fetchAsBlobViaBiliVideoTab(tabId, media, onProgress);
    } catch (ePage) {
      try {
        return await fetchAsBlob(media, referer, onProgress);
      } catch {
...
async function fetchAsBlob(imageUrl, referer = REFERER_XHS, onProgress) {
  const headers = {
    Referer: referer,
    "User-Agent":
  ...
  const isBiliStream = shouldSendBilibiliStreamCookies(imageUrl);
  const credentials = isBiliStream ? "include" : "omit";
  let r;
  try {
  ...
  if (wantsProgress && r.body && typeof r.body.getReader === "function") {
    const cl = r.headers.get("content-length");
    const total = cl ? Number(cl) || 0 : 0;
    ...
    if (isBiliStream) assertBufferMatchesContentLength(loaded, cl);
    return new Blob([out.buffer], { type: ct || "application/octet-stream" });
  }
...
async function fetchBiliDashTrackWithPlayinfoRefresh(
  ...
  async function attemptOrdered(urls) {
    ...
    for (const u of urls) {
      try {
        return await fetchBiliCdnBlobDualContext(tabId, u, referer, onProgress);
      } catch (e) {
        lastErr = e;
      }
    }
...
function biliCoverUploadFilename(title, urlHint) {
  ...
  const ext = pickImageExtFromUrl(urlHint);
  return `${stem}-cover.${ext}`.slice(0, 180);
}

function pickImageExtFromUrl(urlHint, fallback = "jpg") {
  try {
...
/** 图片附件命名：{标题}.ext（单图） / {标题}-N.ext（多图）；抽不到标题时退回 guessFilename */
function buildTitledImageFilename({ title, index, total, urlHint }) {
  const stem = safeBiliClipStem(title);
  if (!stem || stem === "bili-dash") {
    return guessFilename(urlHint, index, "xhs");
  }
  const ext = pickImageExtFromUrl(urlHint);
  const name =
    total > 1 ? `${stem}-${index + 1}.${ext}` : `${stem}.${ext}`;
...
/** PATCH /api/cards/:id 用完整 media 数组覆盖（服务端按 url 复用 file 卡，不会重建已有附件） */
async function patchCardMedia(apiBase, token, userId, cardId, mediaArr) {
  const id = String(cardId || "").trim();
  if (!id) throw new Error("patchCardMedia: 缺少 cardId");
  const r = await fetch(
    appendUserId(`${apiBase}/api/cards/${encodeURIComponent(id)}`, userId),
    {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
      ...
      body: JSON.stringify({ media: mediaArr }),
    }
  );
  const j = await r.json().catch(() => ({}));
  if (!r.ok) {
    const raw = j.error || `更新附件失败 ${r.status}`;
    if (r.status === 401 || r.status === 403) {
      throw new Error("无法更新附件：Token 无效或无写入权限");
    }
    if (r.status === 404) throw new Error("无法更新附件：目标卡片不存在");
    throw new Error(String(raw));
  }
  return j;
}
...
function normalizeUrlForCompare(raw) {
  ...
  try {
    ...
    /** 同一 B 站稿件不同选集（?p=N）视为同链接，触发后续自动合并到同一卡片 */
...
async function runSave(tabId, emit, askDuplicateConfirm) {
  ...
  /** B 站多选集合并：同稿件（同链接/同标题+同作者）命中时，跳过建卡、改为把新选集视频追加到现有卡片附件 */
  let mergeIntoCardId = "";
  /** @type {Array<any>} */
  let mergeExistingMedia = [];
  try {
    ...
    if (dup) {
      /** 只有 B 站多选集（partTitle 非空）才自动合并；单集视频按原逻辑提示用户 */
      if (isBili && dup.cardId && partTitle) {
        mergeIntoCardId = dup.cardId;
        mergeExistingMedia = Array.isArray(dup.media) ? dup.media : [];
      } else {
        const reasonLabel =
          dup.reason === "url"
            ? "同链接"
  ...
  /** 合并模式下：若现有附件名里已含本选集文件名，说明此选集已挂在卡片里，直接结束避免重复上传 */
  const partStemForMerge = partTitle
    ? safeBiliClipStem(partTitle)
    : safeBiliClipStem(title);
  if (mergeIntoCardId) {
    const already = mergeExistingMedia.some((m) => {
      const n = String(m?.name || "");
      if (!n || !partStemForMerge) return false;
      /** 附件名形如 "{stem}-video.mp4" / "{stem}-audio.m4a" / 合并后的 "{stem}.mp4"；
       *  用前缀 + 分隔符判定，避免 "1.1" 误命中 "1.10" */
      return (
        n === partStemForMerge ||
        n.startsWith(`${partStemForMerge}-`) ||
        n.startsWith(`${partStemForMerge}.`)
      );
    });
    if (already) {
      emit({
        type: "done",
        ok: true,
        message: `已在合集卡片里（${partTitle || "当前选集"}）：未重复上传。`,
      });
      return;
    }
  }
  ...
  /** 合并模式：封面已挂在现有卡片里，不再重复上传图片 */
  const skipImages = Boolean(mergeIntoCardId);
  const dashSlots =
    isBili && effectiveDashUrls?.videoUrlsToTry?.length
      ? effectiveDashUrls.audioUrlsToTry?.length
        ...
        : 1
      : 0;
  const totalUploads = (skipImages ? 0 : imgs.length) + dashSlots + vids.length;
  ...
  /** 构造流式下载进度回调：把字节数与分数进度都写进主进度条 */
  function makeStepByteProgress(stepIndex, label) {
    return (loaded, total) => {
      const frac =
      ...
      const pct =
        totalUploads > 0
          ? baseAfterScrape +
            Math.round(
              (uploadSpan * (stepIndex + frac)) / Math.max(totalUploads, 1)
            )
          : 86;
      const text = total
        ? `${label} ${fmtMB(loaded)} / ${fmtMB(total)} MB (${Math.round(
            frac * 100
          )}%)`
        : `${label} 已下载 ${fmtMB(loaded)} MB…`;
      emit({
        type: "progress",
        value: Math.min(pct, 85),
        text,
        bytesLoaded: Number(loaded || 0),
        bytesTotal: Number(total || 0),
      });
    };
  }

  for (let i = 0; i < imgs.length && !skipImages; i++) {
    const sourceIdx = i;
  ...
  if (isBili && effectiveDashUrls?.videoUrlsToTry?.length) {
    ...
    try {
      ...
      dashVideoBlob = await fetchBiliDashTrackWithPlayinfoRefresh(
        ...
        vTry.slice(1),
        makeStepByteProgress(dStep0, "B 站 DASH：下载画面轨")
      );
    ...
    if (effectiveDashUrls.audioUrlsToTry?.length && dashVideoBlob) {
      ...
      try {
        ...
        dashAudioBlob = await fetchBiliDashTrackWithPlayinfoRefresh(
          ...
          { bvid: clipBvid, cid: clipCid },
          aTry.slice(1),
          makeStepByteProgress(dStep0 + 1, "B 站 DASH：下载音轨")
        );
    ...
    /** B 站附件显示名用选集标题优先，便于在同一稿件卡片里分辨不同选集 */
    const clipDisplayTitle = partTitle || title;
    if (dashVideoBlob) {
      ...
      if (dashAudioBlob) {
        ...
        try {
          ...
          media.push({
            ...
            ...(partDurationSec ? { durationSec: partDurationSec } : {}),
          });
          dashHandled = true;
      ...
      if (!dashHandled) {
        try {
          const stem = safeBiliClipStem(clipDisplayTitle);
  ...
  for (let j = 0; j < vids.length; j++) {
    ...
    try {
      ...
      /** 视频文件名优先用帖子标题（B 站多选集则用选集标题）；file 卡「标题」取自附件 name
       *  stripExt 后展示。标题抽不到才退到 URL basename / `xhs-video-N.mp4` */
      const titleForFile = isBili && partTitle ? partTitle : title;
      const titleStem = safeBiliClipStem(titleForFile);
      const filename =
        titleStem && titleStem !== "bili-dash"
          ? vids.length > 1
            ? `${titleStem}-${j + 1}.mp4`
            : `${titleStem}.mp4`
          : guessVideoFilename(url, j, isBili ? "bili" : "xhs");
      ...
      /** B 站走 DASH 分支，这里只命中非 B 站（小红书）普通视频；durationSec 来自 content script 抓的 video 元素 */
      const videoDurationSec = isBili ? partDurationSec : durationSec;
      media.push({
        url: up.url,
        kind: up.kind === "video" ? "video" : "image",
        name: up.name || filename,
        sizeBytes: Math.floor(blob.size),
        ...(videoDurationSec ? { durationSec: videoDurationSec } : {}),
      });
    } catch (e) {
  ...
  if (mergeIntoCardId) {
    emit({ type: "progress", value: 90, text: "追加选集到合集卡片…" });
    try {
      /** 保留既有附件顺序，把新选集视频追加到末尾（按 url 去重，防同一选集重复点剪藏时双写） */
      const existing = Array.isArray(mergeExistingMedia)
        ? mergeExistingMedia
        : [];
      const seenUrls = new Set(
        existing.map((m) => String(m?.url || "")).filter(Boolean)
      );
      const appended = media.filter((m) => {
      ...
      const nVid = appended.filter((m) => m.kind === "video").length;
      const nAud = appended.filter((m) => m.kind === "audio").length;
      let msg;
      if (appended.length > 0) {
        const parts = [];
        if (nVid) parts.push(`${nVid} 个视频`);
        if (nAud) parts.push(`${nAud} 段音频`);
        msg = `成功：已把「${partTitle || "当前选集"}」追加到合集卡片（${parts.join(
          "、"
        ) || "附件"}）。`;
        if (errors.length > 0) msg += `部分附件失败：${errTailText().trim()}`;
      } else if (errors.length > 0) {
        msg = `合集卡片已命中，但新附件全部失败。${errTailText().trim()}`;
      } else {
        msg = "合集卡片已命中，但未抓到可追加的视频（请先在页内播放几秒再试）。";
      }
